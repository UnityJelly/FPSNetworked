...
public class Player_Shoot : NetworkBehaviour {
	...
	void CmdTellServerWhoWasShot (string uniqueID, int dmg)
	{
		...
		go.GetComponent<Player_Health>().DeductHealth(dmg);		//Apply the damage to that plater.
	}

	void CmdTellServerWhichZombieWasShot (string uniqueID, int dmg)
	{
		GameObject go = GameObject.Find(uniqueID);
		go.GetComponent<Zombie_Health>().DeductHealth(dmg);		//Apply the damage to that plater.
	}
}
